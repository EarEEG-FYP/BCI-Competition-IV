...
def  load_edf_signals(path):
    try:
        ...
        n = sig.signals_in_file         #64
        signal_labels = sig.getSignalLabels()  # channel names
        sigbuf = np.zeros((n, sig.getNSamples()[0])) # sampling rate(160) * 61
    ...
    sig._close()
    del sig
    print(sigbuf.transpose(), annotations)
...
def load_physionet_data(subject_id, num_classes=4, long_edge=False):
    ...
    returns (X, y, pos, fs)
        ...
        fs: sample rate
    """
    print("Now in the load physionet data function")
    
    SAMPLE_RATE  = 160
    EEG_CHANNELS = 64
    BASELINE_RUN = 1

    MI_RUNS = [4, 8, 12]  # l/r fist                           # Only taking these runs
    if num_classes >= 4:
    ...
    sub = 'S%03d'
    sub = sub % (subject_id)
    print("MI_RUNS: ", len(MI_RUNS))
    for i_run, current_run in enumerate(MI_RUNS):
        # load from file
        print("MI_RUNS: ****************************************", i_run, current_run)
        path = base_path % (subject_id, current_run)
        print("pathhhhhh: ", path)
        
        print("sub       ", sub)
        print("MI_RUNS: ****************************************", i_run, current_run)
        signals, annotations = load_edf_signals('./DataSet/'+sub+'/' + path) # signal shape = (9760, 64), annotaion= [[0, b'60.2', b'T0']]
    ...
    if num_classes < 3:
        ...
        signals, annotations = load_edf_signals('./DataSet/'+sub+'/' + path)
...
def load_raw_data(electrodes, subject=None, num_classes=4, long_edge=False): # [1],(1,2)
    # load from file
    trials = []
    labels = []
    ...
    for subject_id in subject_ids:   #[1,2,3]
        print("id id id id id id id id id: ", subject_id)
        try:
            print("im in the try block")
