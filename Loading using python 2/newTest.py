...
SAVE = './DataSet_Mat_Format_newChina/'
...
for channel in range(64):
    electrodes = [channel]
    subject = range(1, 110)
    X_, y_ = load_raw_data(electrodes=electrodes, subject=subject, num_classes=nclasses)
    X_ = np.squeeze(X_)
    datasetName = 'Dataset_'+str(channel+1)+'.mat'
    labelsName = 'Labels_'+ str(channel+1) +'.mat'
    sio.savemat(SAVE + datasetName, {'Dataset': X_})
    sio.savemat(SAVE + labelsName, {'Labels': y_})